...
# ensure project root is on import path for fallback data loading
...
def load_transformed_data():
    try:
        ...
        print(f"Warning: could not load transformed data from Postgres: {e}")

    fallback_path = os.getenv('TRANSFORMED_DATA_PATH', os.path.join('etl', 'transformed_shoppers.csv'))
    if os.path.exists(fallback_path):
        print(f"Loading transformed data from fallback CSV: {fallback_path}")
        return pd.read_csv(fallback_path)

    data_path = os.getenv('DATASET_PATH')
    if data_path and os.path.exists(data_path):
        print(f"Loading raw dataset from {data_path} and transforming it")
        from etl.transform import transform_data
        raw_df = pd.read_csv(data_path)
        transformed_df = transform_data(raw_df)
        transformed_df.to_csv(fallback_path, index=False)
        print(f"Saved transformed fallback data to {fallback_path}")
        return transformed_df

    raise RuntimeError('Could not load transformed data from Postgres, fallback CSV, or raw dataset.')
